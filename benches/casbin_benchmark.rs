...
use database_entity::dto::{AFAccessLevel, AFCollabMember, AFPermission};
use dotenv::dotenv;
...
fn casbin_collab_access_control_benchmark(c: &mut Criterion) {
  ...
  for size in [100, 1000] {
    ...
    let target_permission = AFPermission {
      id: Default::default(),
      name: Default::default(),
      access_level: AFAccessLevel::FullAccess,
      description: Default::default(),
    };
    let target_member = AFCollabMember {
      uid: target_uid,
      oid: target_oid.clone(),
      permission: target_permission,
    };

    let target_policy = vec![(target_oid.clone(), vec![target_member])];

    // generate policies
    runtime.block_on(async {
      let collab_members = gen_random_collab_members(&mut rng, size);
      let policies = create_collab_policies(collab_members);
      enforcer
      ...
      enforcer
        .add_policies(create_collab_policies(target_policy))
        .await
        .expect("failed to add collab policies");
    });
...
fn gen_random_collab_members(rng: &mut ThreadRng, n: i64) -> Vec<(String, Vec<AFCollabMember>)> {
  assert!(n > 0);
  let mut collab_members: Vec<(String, Vec<AFCollabMember>)> = Vec::new();
  for _ in 0..n {
    let collab_id = Uuid::new_v4().to_string();
    let mut members: Vec<AFCollabMember> = Vec::new();
    for _ in 0..n {
      members.push(AFCollabMember {
        uid: rng.gen_range(0..n),
        oid: collab_id.clone(),
        permission: gen_random_permission(rng).expect("failed to generate permission"),
      });
    }
    collab_members.push((collab_id, members));
  }
  collab_members
}

/// Generate a random permission.
/// `access_level` is the only field that is used, the rest are left as default.
fn gen_random_permission(rng: &mut ThreadRng) -> Option<AFPermission> {
  let levels = [
  ...
  let access_level = levels.choose(rng)?;
  Some(AFPermission {
    id: Default::default(),
    name: Default::default(),
