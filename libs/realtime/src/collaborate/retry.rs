  ...
  AC: CollabAccessControl,
{
  ...
  ) -> RetryIf<Take<FixedInterval>, SubscribeGroupIfNeed<'a, U, S, AC>, SubscribeGroupCondition<U>>
  {
    ...
    let retry_strategy = FixedInterval::new(Duration::from_secs(2)).take(20);
    RetryIf::spawn(
      retry_strategy,
      self,
  ...
  AC: CollabAccessControl,
{
  ...
  fn run(&mut self) -> Self::Future {
    Box::pin(async {
      ...
      // contains_group and create_new_group should be atomic.
      ...
      } else if self.groups.contains_user(object_id, user).await? {
        return Ok(());
      }
